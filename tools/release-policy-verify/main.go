...
func verifyPolicyCoverage(manifest *buildManifest, policy *deliveryapis.SupportPolicy) error {
	if err := verifyReleaseFoundation(manifest, policy); err != nil {
		return err
	}
	advertised := advertisedVersions(manifest)
	referenced, err := collectPolicyReferences(policy, advertised)
	if err != nil {
		return err
	}
	if err := verifyAdvertisedPolicyReferences(advertised, referenced); err != nil {
		return err
	}
	return verifyKubernetesPolicyReferences(manifest.Resources.K8s.Versions, policy)
}

func verifyReleaseFoundation(manifest *buildManifest, policy *deliveryapis.SupportPolicy) error {
...
func advertisedVersions(manifest *buildManifest) map[string][]string {
...
func collectPolicyReferences(policy *deliveryapis.SupportPolicy, advertised map[string][]string) (map[string]map[string]struct{}, error) {
	...
	for _, support := range policy.Spec.Policies {
		for _, slot := range support.ComponentSlots {
			...
			if !contains(advertised[key], slot.Default.Version) {
				return nil, fmt.Errorf("policy %s default %s:%s is not advertised by the release manifest", support.Name, key, slot.Default.Version)
			}
		}
	}
...
func verifyAdvertisedPolicyReferences(advertised map[string][]string, referenced map[string]map[string]struct{}) error {
...
func verifyKubernetesPolicyReferences(versions []string, policy *deliveryapis.SupportPolicy) error {
	for _, version := range versions {
