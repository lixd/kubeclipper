...
### 12.18 R22 修复轮（2026-09-25，rc.12→rc.15）：R21 全部 8 项发现修复 + 真机复验

**发布链**：commit `9d987785`（8 项修复）→ rc.12 发布（首误发 arm64——发布脚本默认
`go env GOARCH`,Mac 上必须显式 `--arch amd64`;重发同 tag 被**发布器 tag 冲突防护拒绝**
（"package tag conflict ... refusing"——正向验证了防重指机制,遂顺延版本号）→ rc.13
（`9f61c944` sudo 免密前置探测）→ rc.14（`a064cffe` 中转名随机化）→ rc.15（`58a1fc30`
中转目录改用户 home）→ 平台 `upgrade all` 12 槽位全成功（platform API 报 rc.15）。顶层
index digest rc.15=`sha256:450d0980…`。

**修复与复验对照**：

| 发现 | 修复 | 真机复验 |
|---|---|---|
| P1 升级 op 永久 Pending（幽灵锁） | `acquireLock` 驱逐失效持锁者（terminal/已消失,controller.go）——terminal op 不再 reconcile、Pending op 无法 cancel/retry,只能驱逐 | agent 离线时提交升级 → op 立即 Running（不再 Pending 卡死）→ 步骤执行/取消收敛（etcd leader 切换后 ~4min） |
| P1 已删集群操作泄漏（16,595 孤儿） | clustercontroller CRI 对账:DeletionTimestamp 即跳过 + 存在非终结 op 不再新建（controller.go:568 区） | 删除 r22-1m → op 总数 4→0 且 30s×2 稳定（R21 同场景 16,595 且持续增长）；发布器 tag 冲突拒绝亦实证 |
| P1 1M 默认 taint 死锁 | create_cluster.go 默认 taint master→control-plane（kubeadm 本就打 control-plane,遗留 master taint 使 coredns 不可调度） | 1M 不带 --untaint-master 建群 → **Running**（~5min）,节点仅剩 control-plane taint,coredns 2/2 Running |
...
purge/消失,agent 无执行痕迹,33min 无进展;取消后收敛）——疑似任务 purge 与执行竞争,需单独立项。
...
### 12.21 R24-B 追加轮（2026-09-26，rc.21 2778f818）：kcctl 命令余量 + 小项（3-04/3-09/3-15/3-19/3-24/3-27/2.1-13/2.1-17/4-12/4-20）+ feature-gates 缺陷修复

**① 2.1-13 feature-gates：发现产品缺陷并修复（rc.21，`2778f818`）**。原计划"复跑 feature-gates 透传"，首次建群（`--feature-gates=HPAScaleToZero=true,InPlacePodVerticalScaling=true`）即失败：agent 日志 `error: [featureGates: Invalid value: {...}: HPAScaleToZero is not a valid feature name., ...]`——kubeadm 1.37 对 ClusterConfiguration 的 featureGates map 全量拒绝。用 kubeadm `init --dry-run` 建立 gate 名探针（把候选插进 ClusterConfiguration.featureGates，以 "not a valid feature name" 判定）批量验证：**该版本 kubelet `--help` 列出的 ALPHA/BETA gate（CSIVolumeHealth/APIServingWithRoutine/ClientsAllowCBOR/AtomicWriteVolumeUserFields/CloudControllerManagerWebhook/CPUManagerPolicyAlphaOptions，以及 AllAlpha）全部被 kubeadm 拒绝** → 结论：v1beta4 下该字段不可用，产品的渲染位置错误。修复：模板不再渲染 ClusterConfiguration.featureGates，改为 apiServer/controllerManager/scheduler 的 `extraArgs`（v1beta4 list 形 `- name: feature-gates / value: "k=v,..."`，旧版本回落 map 形）+ KubeletConfiguration.featureGates（map）；Go 侧新增 `FeatureGatesArg`（排序确定性）并在 Render/renderTo 双处派生；渲染断言测试（ClusterConfiguration 无 featureGates、三组件 arg 各一次、kubelet 段命中）。真机复验：r24-1m（v1.37.0，`--feature-gates=APIServingWithRoutine=true --kubeadm-init-ignore-preflight-errors=Swap`）**Running**；节点侧 `/etc/kubernetes/manifests/kube-{apiserver,controller-manager,scheduler}.yaml` 均含 `--feature-gates=APIServingWithRoutine=true`、`/var/lib/kubelet/config.yaml` featureGates 命中。**透传缺陷的完整修复链：候选→发布→升级→真机建群→节点参数取证。**

**② 建群过程新观察（记录项）**：①平台自动创建的 `kc-package-registry` 资源 `scheme: https` 而共享 5003 实为 HTTP → 用它 `--image-registry` 建群在 kubeadm init 镜像拉取阶段失败（`server gave HTTP response to HTTPS client`）；改用手工资源 `kc-image-registry`（http+skipVerify）即成功。建议部署侧写入正确 scheme 或与端口协商（gaps 行 16，未修）。②`kcctl delete cluster -F` 在无 TTY（stdin EOF）时 FATAL 打印 goroutine 堆栈，全局 `-y/--assumeyes` 未被该路径采纳（须 `echo yes |`）；③force delete 在有 op 在飞时被正确拒绝（`can't delete cluster when cluster is Updating`，正向守卫）；④`drain` used 拒绝文案拼写 "draind"。

**③ kcctl 命令余量（安全批真机，rc.20）**：3-09 User selector 三态（labelSelector/fieldSelector/空集）；3-15 drain 不存在节点干净报错、在用节点 used 保护（`is used by the cluster r24-1m`）；3-19 repository 列表（只读）+ delete 不存在 tag fetch-first 干净失败（零变更；对共享 5003 的真实 clean/delete 按"只增 tag"策略排除）；3-24 template(-o yaml)→validate "valid"；3-27 生成/非法 YAML 秒级拒绝/顶层 initialPassword 拒绝（R23 文案）；4-12 kubeconfig 三态（建群中 500 no-master-node、不存在 500 not-found、就绪 200；状态码宜 404/503 为记录项）；3-04 join 由 R13-C4（错误凭据/CA）+R21（防重入）+2.2-07（drain→join 重建）合围。

**④ 4-20 Addon 失败/清理闭环（r24-1m 真机）**：非法 config（scName `Bad_Name!`）→ 500 `invalid name of storage class` 零副作用（宜 400，记录项）；正向安装 nfs-csi（serverAddr 172.16.131.146、sharedPath /tmp/nfs-share、scName r24-sc）→ SC 与 csi-nfs controller/node pods 全 Running；安装 op 末步 `checkCSIHealth`（**declared timeout 3m**）实际 Running >6min，cancel 后协作收敛 **Canceled**（custom step 的 deadline 未强制=记录项）；排队卸载 op 在锁释放后 **Succeeded**，集群侧 SC 消失、nfs pods 0 残留。
