...
**更新（R20，rc.11 `9a1dbb75`，2026-09-24）**：B1 最后两项余量收口——①升级中故障注入矩阵
四场景真机闭环（server 健康探测失败自动恢复、kcctl 替换后探针失败 restoreKcctl、console
探针失败 restoreConsole、中断后再执行 skip+补齐收敛，restore 均逐字节恢复/三节点基线一致，
见 R7 报告 §12.16）；②`--version` 在线正向下载+checksum 以本地 GitHub 镜像法闭环（自签 CA
+ /etc/hosts + 443 HTTPS 供给与真实 release 同构的 URL；`--version v2.0.3` 下载+校验+幂等
skip 全链路 rc=0，`.sha256` 篡改被 checksum mismatch 拒绝；临时证书按约束即用即删）。
**B1 关闭条件全部满足**（§2.5）。稳定版发布结论仍为 Blocked（B6 门禁真实放行/阻断复验与
首个验收记录等发布侧余量未清）。
...
| 编号 | 内容                          | 证据性质                                   | 初始实施状态 |
| ---- | ----------------------------- | ------------------------------------------ | ------------ |
| B1   | 平台升级产物与 OCI 发布不一致 | 已确认代码和发布契约缺陷                   | 已实施（2026-09-20，E2E 复验见 R7 报告 §11）；step 2 console/kcctl 已实施并真机闭环（2026-09-24 R19，rc.10 12 槽位+幂等 skip/重装+负向三例，见 R7 报告 §12.15）；余量两项已于 R20 收口（故障注入矩阵四场景 + `--version` 本地镜像法正向下载+checksum 负向，见 R7 报告 §12.16）。**B1 关闭** |
...
> **执行状态（2026-09-24，R19/R20，R7 报告 §12.15/§12.16）**：内网 OCI 升级（`upgrade all` 12 槽位
...
> 已移除。R20 收口最后两项余量：升级中故障注入矩阵四场景（健康探测失败自动恢复、kcctl/console
> 替换后探针失败 restore、中断后再执行 skip+补齐）与 `--version` 在线正向下载+checksum
> （本地 GitHub 镜像法，负向 checksum mismatch 拒绝）。**B1 关闭条件全部满足，B1 关闭。**
