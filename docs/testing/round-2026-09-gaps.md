...
| 16 | （新增 R24）平台自动创建的 registry 资源 | `kc-package-registry` scheme 默认 https 指向 HTTP registry | 部署自动生成的 `kc-package-registry`（host 172.16.131.146:5003、**scheme: https**、无 skipVerify）与共享 registry 实际协议（HTTP）不符：用它做建群 `--image-registry` 时 containerd 报 `server gave HTTP response to HTTPS client`，建群在 kubeadm init 镜像拉取阶段失败（R24 实测；改用手工建的 `kc-image-registry`（http+skipVerify）即 Running）。**未修**（记录+建议）：资源 scheme 应与部署配置/端口协商或由部署侧显式写入（R7 报告 §12.21） |
