...
| 顺序 | Case | 缺口 | 完成条件 |
|---:|---|---|---|
| 1 | `1.2-07` | HA 故障窗口完整验收 | R4 已完成 3 Server/3 etcd 部署，并分别停止/恢复单 Server、单 etcd；仍缺故障窗口中的 Watch、运行中 Operation 和 Console 入口证据 |
| 2 | `1.1-03`、`1.1-08`、`2.1-23` | 纯离线 bundle 真机闭环 | export、拷贝、重复 import 后，在断公网环境完成平台部署、建群、Addon、升级和删除；保存网络封锁与 digest 证据 |
| 3 | `2.6-09`、`2.6-10` | Delivery Policy 对建群的约束 | R4 已跑通 template/validate/diff/apply/get，并验证非法 selection 拒绝；仍缺白名单内/外建群以及缺失 slot 在创建 Operation 前拒绝。未知 slot 当前实现按可扩展字段接受，需先确认产品语义 |
