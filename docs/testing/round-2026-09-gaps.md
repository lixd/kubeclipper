...
| 5 | `2.2-03`、`2.2-09`、`2.2-10` | Master 增删 | 添加后 control-plane/etcd quorum 正常；移除后 etcd member、证书、VIP 和节点角色正确收敛。**更新（R17，2026-09-23，rc.8 重 deploy 平台真机）：定性为产品缺口**——双 master 集群（dev-3+dev-4，v1.35.8/calico v3.29.6/containerd 1.7.29，payload 显式 `imageRegistry`）Ready 后，Master add/remove 均 400 `invalid node role`；代码定位 `makeMasterCompare`（pkg/clusteroperation/node.go:101-104）无条件 `return ErrInvalidNodesRole`（`// support later`），doMakeOperation master 分支同拒，etcd member remove/证书清理实现缺失。同轮 worker add/remove 正向首证（v2 operation 模型）：dev-2 入群 Ready（28 OperationTasks 按 stepID×节点派发）、移除后集群标签/hosts 清理、无二进制残留（`/tmp/.k8s` 残留为小观察项）；Installing 中 force delete 400 须先 `POST /operations/{name}/cancel`（body 平铺 uid+resourceVersion）→ Canceled → InstallFailed → force delete 成功。完成条件不变：实现 master 增删后按 2.2-09/10 验收 quorum/证书/VIP 收敛 |
...
| 10 | （新增）创建 payload `imageRegistry` | 离线建群未显式指定 imageRegistry 时慢失败且无创建前校验 | **新发现（R17，2026-09-23，rc.8 真机）**：clean 后全新节点离线建群，payload 无 `imageRegistry` → kubeadm.yaml 不渲染 `imageRepository`（默认 registry.k8s.io），containerd certs.d 无本地镜像映射 → `kubeadm init` 从公网拉 7 镜像（kube-apiserver/controller-manager/scheduler/proxy/coredns/etcd/pause）i/o timeout，单 attempt ~18 分钟，5400s deadline 内循环重试后 Failed，创建前零校验零提示；`DownloadImage` 仅 Upgrade 路径消费且 OCI 交付下直接报错（pkg/scheme/core/v1/k8s/cluster.go:397 `"upgrade Kubernetes image archives are not supported with OCI package delivery"`），创建路径无镜像 load 步骤。R15/R16 建群成功系节点 containerd 遗留镜像掩护（R16 清理 /var/lib/containerd 后暴露）。**修复已实证**：payload `"imageRegistry": "kc-package-registry"`（Registry 对象名，deploy 时自动创建，http/5003）→ `ResolveImageRegistry` 同源驱动 kubeadm `imageRepository: 172.16.131.146:5003` 与 containerd certs.d hosts.toml（http endpoint）→ 双 master 建群 20/20 Succeeded。完成条件：payload 未显式指定时创建前显式拒绝或默认解析到 package registry（文档/CLI 同步提示），离线环境错误秒级暴露而非 ~18 分钟 |
