...
| 13 | （新增 R24）登录限流零值配置 | 首次口令错误即永久锁死账号（实测锁死平台 admin） | deploy-config 的 authentication 段经 Omitempty 零值往返写入 `authenticateRateLimiterMaxTries: 0`/`Duration: 0s`；运行时限流器 `count>=0` 恒真，计数键 TTL=0 永不过期 → 一次口令错误后账号被永久 429（含正确口令），文案 "try again in 0 minutes"。**已修复（R24，`954a1865`，rc.17）**：部署侧 `RestoreZeroDefaults` 回填运维旋钮默认值；运行时 `MaxTries<=0` 视为禁用、`Duration<=0` 回退 10m。复验：坏配置（0/0s）下错误 2 次后正确口令仍 200；5/2m 逐项验证阈值/隔离/窗口恢复/成功清零（R7 报告 §12.20） |
| 14 | （新增 R24）MFA/验证码路径 | 启用即三节点 crash-loop；验证码登录无路可通 | ①`mfa/sms` 包无任何导入，provider 工厂未注册，配置 `mfaOptions` 即启动失败 crash-loop（实测平台 API 全宕）；②验证码校验 `url.Values` 零值 Set 必 panic；③fake provider 限流标记与验证码共用 key，etcd cache Set 撞 AlreadyExists → 码不落库不打印且永久限流；④tokencontroller 写完状态即 return，stale lister 下不再入队 → 过期透明键长生。**全部已修复（R24，`64775c6a`/`468fb5bd`，rc.18/rc.19）**：空白导入+回归测试、`url.Values{}`、标记独立键+清理残留码、deadline 自算+落穿+恒 RequeueAfter。复验：全流程 428→发码→校验→token，错码/复用/过期/重发限流逐项通过，fake_sms 键 0 残留（R7 报告 §12.20） |
...
| 7 | `4-08`～`4-08d`、`4-18` | 用户、登录和 RBAC | R6 已完成临时 user/role CLI CRUD、重复名拒绝和正确密码登录；内置只读用户越权 403 通过，但自定义 role 登录后读取 Cluster/Node 仍 403，需修复绑定授权并补 enable/disable、验证码、Token 和限流闭环 |。**已闭环（R24，rc.17-rc.20，R7 报告 §12.20）**：4-08 用户/角色 CRUD+登录记录（记录落库修复）、4-08b RBAC（R7 已闭环）、4-08c 验证码全流程（含过期/复用/重发限流；第三方 OAuth 归第 7 章）、4-08d token 生命周期（登录签发/列表/撤销/TTL 清理；审计明文泄漏已修复）、4-18 限流全矩阵（阈值/隔离/窗口/清零；零值配置锁死已修复）。记录项：弱口令建用户被接受、logout 需 JSON 头（406）、重发限流 500、无 token 创建/删除路由 || 7 | `4-08`～`4-08d`、`4-18` | 用户、登录和 RBAC | R6 已完成临时 user/role CLI CRUD、重复名拒绝和正确密码登录；内置只读用户越权 403 通过，但自定义 role 登录后读取 Cluster/Node 仍 403，需修复绑定授权并补 enable/disable、验证码、Token 和限流闭环 |。**已闭环（R24，rc.17-rc.20，R7 报告 §12.20）**：4-08 用户/角色 CRUD+登录记录（记录落库修复）、4-08b RBAC（R7 已闭环）、4-08c 验证码全流程（含过期/复用/重发限流；第三方 OAuth 归第 7 章）、4-08d token 生命周期（登录签发/列表/撤销/TTL 清理；审计明文泄漏已修复）、4-18 限流全矩阵（阈值/隔离/窗口/清零；零值配置锁死已修复）。记录项（更新后）：logout 需 JSON 头（406，客户端行为）、无 token 创建/删除路由、第 5 次错口令 401+reason；**弱口令已由 R28 口令策略闭环、MFA 重发限流已由 R26 修为 429** |
| 8 | `4-07` | Console 核心 E2E | 登录、建群、升级、备份、删除、Operation 进度和失败原因展示与 API 状态一致 |
| 9 | `3-09`、`3-10`、`3-12`、`3-15`、`3-19`、`3-24`、`3-25`、`3-27` | 未覆盖或未闭环的 `kcctl` 命令 | R6 已补 get 资源扫尾、registry list/image/非法 push、drain、completion、无效升级包和部分登录/RBAC；仍需每条命令成功/典型失败闭环，修复 `get --watch`，补 cancel 自动收敛、valid registry 生命周期、login TLS 和 deploy config 优先级 |**已闭环（R21 + R22 + R24/§12.20 + R24-B/§12.21 + R26/§12.23）**：每条命令成功/典型失败路径逐条真机——get 资源含 User label/field selector 三态、registry 列表与 delete 负向、drain used 保护与重复执行、delivery-policy validate、deploy config 生成/非法拒绝/顶层 initialPassword 拒绝、operation cancel 自动收敛（R8-R10 修复后复验）、login 正确/错误路径、login TLS 与优先级（R16）；3-10 `get --watch` 已由 R7 batch-3 修复并复验 |
| 10 | `5-13`、`5-14` | 重复提交与超时收敛 | 重复请求不产生并发副作用；timeout 后 Task、Cluster 和 ExecutionLock 按既定语义收敛 |**已闭环（R25，rc.22，§12.22）**：5-13 连续两次相同 addon 安装 → 第二次 400（无并行重复 op）；5-14 操作期限（R16）+ **step 级 timeout 修复并真机复验**（健康检查步 ~3min/次超时、op ~4min 自动收敛 Failed；修复前挂到 90min 操作期限） |
