...
| 11 | （新增 R21）`cluster upgrade` 派发 | 升级操作 Pending 死锁 | 提交时 agent 离线 → operation 永久 Pending：任务不派发、agent 恢复不补派、cancel/retry 拒绝 Pending、须重启 kc-server 才能取消；agent 在线新建 op 同样 Pending（集群卡 Upgrading）。修复方向：Pending 超时/重派机制 + cancel/retry 对 Pending 可用 + 派发重试队列 |
| 12 | （新增 R21）已删集群操作泄漏 | InstallComponents 无限增殖 | clustercontroller CRI-registry 对账对已删集群（僵尸 Running 无 deletionTimestamp）持续创建孤儿 Pending 操作（实测 16,595 个）；operations API 无 DELETE 路由无法清理。修复方向：Terminating/删除后停止对账 + status 更新失败退避 + 对象终态清理 |
