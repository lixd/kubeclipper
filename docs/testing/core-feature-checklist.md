...
| 3-15 | `kcctl drain` | ✅ | R6 空闲 Agent drain；R24 补验 used 保护（`... is used by the cluster <name>`）与不存在节点报错；**R26 修正文案拼写（"draind"→"drained"）并让 drain/delete 采纳全局 `-y/--assumeyes`**；无 TTY 时确认提示改为干净拒绝（不再 FATAL 堆栈）。边界：force 与重复执行未单独跑（-F 语义=强制删在用节点，R18 master remove 已覆盖同族路径） |
