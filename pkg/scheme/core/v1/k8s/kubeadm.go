...
func (stepper *Package) installResolved(ctx context.Context, opts component.Options) ([]byte, error) {
	if err := downloadAndUnpackResolvedPackage(ctx, stepper, opts); err != nil {
		return nil, err
	}
...
func downloadAndUnpackResolvedPackage(ctx context.Context, stepper *Package, opts component.Options) error {
	...
	if configs == "" {
		return fmt.Errorf("resolved k8s configs content is missing")
	}
